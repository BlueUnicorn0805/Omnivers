...
public class ActivityCatering extends BaseActivity implements View.OnClickListener, GeocodeAddressResultReceiver.OnReceiveGeocodeListener {
    public static final String TAG = ActivityCatering.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setTitle("Details of Catering");
        ...
        editThemeName = findViewById(R.id.editThemeName);
        editContactName = findViewById(R.id.editContactName);
        editNumOfPeople = findViewById(R.id.editNumOfPeople);
        ...
        spinnerEatTimeOpts = findViewById(R.id.spinnerEatTimeOpts);
        ...
        spinnerEatTimeOpts.setAdapter(spinnerArrayAdapter);
        ...
        txtFee.setText(String.format("$%.2f Non Refundable", restaurant.getCatFee()));
        ...
        // Restore inputs
        try {
            JSONObject jsonCater = new JSONObject(appSettings.getCaterInputs());
            editThemeName.setText(jsonCater.getString("theme"));
            editContactName.setText(jsonCater.getString("name"));
            editNumOfPeople.setText(jsonCater.getString("num"));
            editAddress.setText(jsonCater.getString("addr"));
            editAppt.setText(jsonCater.getString("apart"));
            editFloor.setText(jsonCater.getString("floor"));
            editCSZ.setText(jsonCater.getString("csz"));
            editPhone.setText(jsonCater.getString("phone"));
            editNotes.setText(jsonCater.getString("notes"));
    ...
    public void onClick(View view) {
        ...
        if (viewId == R.id.btnRequest) {
            hideKeyboard(editThemeName);
            hideKeyboard(editContactName);
            hideKeyboard(editNumOfPeople);
            ...
            String theme = editThemeName.getText().toString().trim();
            String cName = editContactName.getText().toString().trim();
            String numofPeople = editNumOfPeople.getText().toString().trim();
            String date = DateUtil.toStringFormat_25(calendarDOB.getTime());
            String dateValue = strDOB;
            String time = timeOptions.get(spinnerEatTimeOpts.getSelectedItemPosition());
            String timeValue = timeValues.get(spinnerEatTimeOpts.getSelectedItemPosition());
            ...
            if (TextUtils.isEmpty(theme) || TextUtils.isEmpty(cName) || TextUtils.isEmpty(numofPeople) ||
                    TextUtils.isEmpty(date) || TextUtils.isEmpty(address) || TextUtils.isEmpty(csz)) {
            ...
            startActivity(new Intent(mContext, ActivityCaterHistory.class));
    ...
    private void submitCatering(double toLat, double toLon) {
        ...
        try {
            jsonCater.put("theme", theme);
            jsonCater.put("name", cName);
            jsonCater.put("num", numofPeople);
            jsonCater.put("addr", address);
            jsonCater.put("apart", apart);
            jsonCater.put("floor", floor);
            jsonCater.put("csz", csz);
            jsonCater.put("phone", phone);
            jsonCater.put("notes", notes);

            appSettings.setCaterInputs(jsonCater.toString());
        } catch (JSONException e) {
            e.printStackTrace();
        }
        ...
        paramMap.put("receipt", cName);
        paramMap.put("theme", theme);
        paramMap.put("numofpeople", numofPeople);
