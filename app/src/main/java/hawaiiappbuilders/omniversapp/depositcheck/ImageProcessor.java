...
import hawaiiappbuilders.omniversapp.depositcheck.helpers.Utils;
...
// && ((!autoMode && !previewOnly) || (autoMode && qrOk))
    ...
    private boolean detectPreviewDocument(Mat inputRgba) {
        ...
        if (quad != null) {
            ...
            double xCenter = (quad.points[0].x + quad.points[2].x) / 2;
            double yCenter = (quad.points[0].y + quad.points[2].y) / 2;

            Point pointCenter = new Point(xCenter, yCenter);

            Log.d(TAG, "center point: (" + pointCenter.x + "," + pointCenter.y + ")");
